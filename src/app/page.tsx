  ...
  return (
    <main onClick={startHum} className="relative w-full h-screen overflow-hidden bg-black">
      <div id="canvas-container" className="absolute inset-0 z-0">
        <Suspense fallback={<div className="flex items-center justify-center w-full h-full text-[#0ff0fc] hud-font">Initializing Cosmos...</div>}>
          <Scene />
        </Suspense>
      </div>
      ...
      <div className="absolute inset-0 z-10 pointer-events-none p-8 flex flex-col justify-between perspective-1000">
        <header className="flex justify-between items-start">
          ...
          >
            <h1 className="hud-font text-[var(--neon-blue)] text-xl font-bold">NEBULA OS // COSMIC</h1>
            <p className="text-xs text-slate-400 mt-1">OPEN WORLD SIMULATION v2.0</p>
          </motion.div>
          ...
          >
            <button
              ...
              className="text-xs font-bold text-[var(--neon-purple)] hover:text-white transition-colors"
            >
              TIME REVERSAL (UNDO)
            </button>
          </motion.div>
        ...
        <footer className="flex justify-between items-end pb-8">
          ...
          >
            <div className="text-[10px] text-[var(--neon-blue)] tabular-nums uppercase tracking-widest">
              GPGPU GRAVITY: <span className="text-white">ONLINE</span>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, x: pointer.x !== 0 ? parallaxX * 0.5 : xTransform.get() * -0.5, y: pointer.y !== 0 ? parallaxY * 0.5 : yTransform.get() * -0.5 }}
            transition={{ type: 'spring', stiffness: 100, damping: 30 }}
            className="stark-panel p-4 pointer-events-auto max-w-sm mr-56"
          >
            <h2 className="hud-font text-sm text-[var(--neon-blue)] mb-2">SPATIAL CONTROLS</h2>
            <ul className="text-xs text-slate-300 space-y-1">
              <li><span className="text-[var(--neon-purple)] font-bold">1-Tap:</span> Select Planet</li>
              <li><span className="text-[var(--neon-purple)] font-bold">Fist:</span> Destroy Selection</li>
              <li><span className="text-[var(--neon-purple)] font-bold">2-Swipe:</span> Rotate Universe</li>
              <li><span className="text-[var(--neon-purple)] font-bold">3-Pinch:</span> Zoom Camera</li>
            </ul>
          </motion.div>
        </footer>
      </div>
